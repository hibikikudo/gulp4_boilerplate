...
function scripts() {
    return gulp
        ...
        .pipe(sourcemaps.init())
        .pipe(
            order([paths.scripts.core, paths.scripts.app])
        )
        .pipe(babel({
            presets: ['env']
        }))
...
gulp.task("build", gulp.series(gulp.parallel(scripts, images, sass_compress, html), cleanMapFiles));
gulp.task("clean", cleanMapFiles);
gulp.task("eslint", esLint);
gulp.task("html-lint", htmlLint);
gulp.task("imagemin", images);
gulp.task("sass-compress", sass_compress);
